...
- **WHEN** the model output omits presenter identity (or returns whitespace-only presenter text)
- **THEN** the parser SHALL set `presenter_name` to null
- **AND** persisted extraction data SHALL store `presenter_name` as NULL
