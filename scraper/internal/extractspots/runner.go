...
func Run(ctx context.Context, repo *repository.Repository, opts Options) ([]Result, error) {
	rows, err := repo.ListTranscriptionsPendingExtraction()
	if err != nil {
		return nil, fmt.Errorf("failed to select pending transcriptions: %w", err)
	}
	if len(rows) == 0 {
		return nil, nil
	}

	client := genaiclient.NewClientWithEndpoint(strings.TrimSpace(opts.APIKey), strings.TrimSpace(opts.Model), strings.TrimSpace(opts.Endpoint))
